...
public class Zoom extends AbstractTool {
	...
	public void apply() {
		...
		checkRange(range);
		final double[] limits = mRenderer.getZoomLimits();
		...
		if (mRenderer.isZoomXEnabled()) {
			final double newXMin = centerX - newWidth / 2;
			final double newXMax = centerX + newWidth / 2;
			if (!limited || limits[0] <= newXMin && limits[1] >= newXMax) {
				setXRange(newXMin, newXMax);
			}
		}
		if (mRenderer.isZoomYEnabled()) {
			final double newYMin = centerY - newHeight / 2;
			final double newYMax = centerY + newHeight / 2;
